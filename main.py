...
def show():
    ...
    for p in places:
        html += f"<div style='background:white; border-radius:30px; padding:30px; box-shadow:0 25px 50px rgba(0,0,0,0.15); text-align:center;'><h2 style='color:#ff6b6b;'>{p.get('name','')}</h2><h3 style='color:#54a0ff;'>{p.get('city','')}</h3><p>{p.get('description','')}</p></div>"
